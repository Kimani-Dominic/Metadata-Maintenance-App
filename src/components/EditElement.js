    ...
    dataElement: {
        ...
        params: {
            fields: [
                ...
                'domainType', 'valueType', 'aggregationType', 'categoryCombo',
                'formName', 'decimals', 'zeroIsSignificant'
            ],
        },
    },
};
...
const EditElement = ({ onSuccess }) => {
    ...
    if (error) {
        return (
            <NoticeBox title='Error loading data element' error>
                {error.message}
            </NoticeBox>
        );
    }
    ...
    return (
        <div>
            ...
            <Form
                dataElementId={id}
                initialValues={data.dataElement}
                onSuccess={onSuccess}
            />
        </div>
    );
